...
export function isValidDateString(date) {
  // commenting this as it is defined but not used later to fix the problem- [no-unused-vars]
  ...
  if (
    ...
    return false; // if there are too few or too many digits in each segment somthing is wrong
...
export function generateFlightId(airline) {
  ...
  // Ensure airline name has at least 2 characters
  if (name.length < 2) {
    return undefined;
  }
  ...
  return airline
    .substring(0, 2)
    .toUpperCase()
    .concat(digits[0].toString(), digits[1].toString(), digits[2].toString());
}
